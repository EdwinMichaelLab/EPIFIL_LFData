...
%% Description: to calculate weighgted percentile for Peneng
...
weight=0;
Frequency=0;
load('Data_PNG_modelonly.mat')
y(isnan(y))=0;
a0=length(nonzeros(y(:,3)));
num0(1:a0,1)=y(1:a0,3);
load('Data_Peneng_LYMPHASIM.mat')
x(isnan(x))=0;
b0=length(nonzeros(x(:,1)));
num0(a0+1:a0+b0,1)=x(1:b0,1);
load('Data_Peneng_TRANSFIL_Scen0.mat')
y(isnan(y))=0;
...
save(sprintf('Allmodel_95per_Peneng.mat'),...
        'd0','d1','d2','d3','d4');