...
%% Description: to calculate weighgted percentile for Alagramam
...
%% for Alagramam Scenario0
weight=0;
Frequency=0;
load('Data_IND_modelonly.mat')
y(isnan(y))=0;
a0=length(nonzeros(y(:,2)));
num0(1:a0,1)=y(1:a0,2);
load('Data_Alagramam_LYMPHASIM.mat')
x(isnan(x))=0;
b0=length(nonzeros(x(:,1)));
num0(a0+1:a0+b0,1)=x(1:b0,1);
load('Data_Alagramam_TRANSFIL_Scen0.mat')
y(isnan(y))=0;
...
%% for Alagramam Scenario1
weight=0;
Frequency=0;
load('Data_Alagramam_EPIFIL.mat')
x(isnan(x))=0;
...
load('Data_Alagramam_TRANSFIL.mat')
x(isnan(x))=0;
...
save(sprintf('Allmodel_95per_Alagramam.mat'),...
        'd0','d1','d2','d3','d4');